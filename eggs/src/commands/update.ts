 ...
 */
async function getLatestVersionOfGitHubRepo(
  owner: string,
  repo: string,
): Promise<string> {
 ...
 * @returns {string} The latest version
 */
async function getLatestVersionFromNestRegistry(
...
export const update = new Command()
  ...
  .action(async () => {
    ...
    let pathToDepFile = "";
    try {
      pathToDepFile = await Deno.realPath("./" + dependencyFilename);
    } catch (err) {
    ...
    const dependencyFileContents: string[] = decoder
      .decode(await Deno.readFile(await Deno.realPath(pathToDepFile)))
      .split("\n")
      .filter((line) => {
    ...
    const requestedDependencies: string[] = args.slice(
      args.findIndex((val) => val === "--deps") + 1 || 100,
    ); // 100 to be empty when not found
    if (args.indexOf("--deps") !== -1 && requestedDependencies.length === 0) {
      console.error(
        "Provide dependencies to update when using --deps. Exiting...",
      );
      Deno.exit(1);
    }
    ...
    for (const line of dependencyFileContents) {
      ...
      if (
        requestedDependencies.length &&
        requestedDependencies.indexOf(name) === -1
      ) {
        continue;
      }
      ...
      if (
        importedVersion.indexOf("v") === 0 &&
        latestRelease.indexOf("v") === -1
      ) {
      ...
      if (
        importedVersion.indexOf("v") === -1 &&
        latestRelease.indexOf("v") === 0
      ) {
      ...
      // Basic safety net
      if (
        !name || !importedVersion || !latestRelease ||
        importedVersion === latestRelease
      ) {
        continue;
      }
    ...
    let dependencyFile = decoder.decode(await Deno.readFile(pathToDepFile));
